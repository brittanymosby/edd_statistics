...
(Recall that the p-value is the probability of obtaining a test statistic that is *more extreme* than the observed value, assuming the null hypothesis is true.)
...
#We still need the test statistic here-- and instead of computing using the Console, we can also enter the sample data directly into the `pnorm` function:

2*pnorm(1350, mean=1400, sd=145/sqrt(28), lower.tail=T)
```

2b. Compare the p-value to our alpha (which we will assume to be .05, unless otherwise stated). Is p LESS than alpha? 
