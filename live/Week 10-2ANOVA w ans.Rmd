...
```{r}
#Enter the filepath to your working directory in the () below.
setwd()
```

Let's take a look at the sleep and exercise data from async. This code chunk will read in the data and then create a two-way ANOVA of Timing and Intensity's potential effect(s) on sleep.
To run a two-way ANOVA test, we will enter the data like this:
`aov(DV ~ A + B + A:B, data=DATANAME)`

```{r}
#Let's look at the sleep data from async
sleepydata <- read.table("sleepdata.txt", header=T, sep="\t")

#The dependent variable is sleep. Timing is factor A and intensity is factor B. To represent the interaction we use a colon (:)

sleep.aov <- aov(Sleep ~ Timing + Intensity + Timing:Intensity, data=sleepydata)
summary(sleep.aov)
```
Based on this table, the timing has a significant effect but also there is a significant interaction between timing and intensity. We can visualize this with an interaction plot.

```{r}
#Factor A, Factor B, DV

interaction.plot(sleepydata$Intensity, sleepydata$Timing, sleepydata$Sleep,
                 ylim=range(15,45), xlab="Intensity",
                 ylab="Sleep",
                 col=c("blue","red"), main="Plot of Timing x Intensity")
```
There is a problem because R is ordering Low, Medium and High in alphabetical order but that doesn't make sense for this problem. However, we can force R to order the labels how we want -- Low, Medium and High by creating a new `factor`.

```{r}

intensereorder <- factor(sleepydata$Intensity, levels=c("Low","Medium","High"))

#Now we will replace `Intensity` with the new `intensereorder`

interaction.plot(intensereorder, sleepydata$Timing,  sleepydata$Sleep,
                 ylim=range(15,45),xlab="Intensity",
                 ylab="Sleep",
                 col=c("red","blue"), main="Plot of Timing x Intensity")
```
Since there is a significant interaction, we don't use eta squared, instead we look at simple main effects. Basically you will rerun ANOVA for each level of the significant factor(s). You do this for factors with more than 2 levels (if a factor has only 2 levels, then your main effect from your two-way ANOVA table already has the info you need).

Let's look at each level of `Timing`. We will use `subset` to create datasets containing only one level of `Timing` and examine each one of those. 

This code creates subsets that only includes "morning" exercisers & one for "evening"

```{r}
#Subset with Morning exercisers only
sleep_AM <- subset(sleepydata, Timing == "Morning")

#Subest with Evening exercisers only 
sleep_PM <- subset(sleepydata, Timing == "Evening")
...
```{r}
#Sleep is still our dependent variable and now the only factor is intensity of exercise.
...
summary(sleepPM.aov)
```
*IMPORTANT NOTE!* For our follow-up simple main effects, we must adjust our value for alpha. We divide alpha up by how many comparisons (or one-way ANOVAs) we're running. In this case - we're running two (one for evening, one for morning). So, $$\alpha = \frac{.05}{2} = .025$$. THAT is the threshold we will judge our simple main effects tests by (NOT p<.05). 
**NOTE: we ONLY use this adjusted alpha value for these follow-up (or post hoc) one-way ANOVA tests. We still use p<.05 for the initial two-way ANOVA.**

What does it all mean?

Looking at output of the follow up one-way ANOVAs we see that the morning subset DID contain a significant effect (p<.025) of intensity on sleep, but the evening subset did NOT (p>.025). 
...
```
Tukey analysis: there is a significant difference in the time to fall asleep for morning exercisers of Low intensity vs. the High intensity groups. There was NOT a significant difference between Medium and High.
...
```{r}
dose.reordr <- factor(my_data$dose, levels=c("D0.5", "D1", "D2"))

interaction.plot(dose.reordr,  my_data$supp, my_data$len,
                 ylim=range(1,35),xlab="Dose Amount",
                 ylab="Tooth Length",
                 col=c("purple","orange"), main="Plot of Dose x Method")
```
...
```{r}
my_data_half <- subset(my_data, dose == "D0.5")
my_data_one <- subset(my_data, dose == "D1")
my_data_two <- subset(my_data, dose == "D2")
...
```
```{r}
half.aov <- aov(len ~ supp, data=my_data_half)
summary(half.aov)
```
```{r}
one.aov<- aov(len ~ supp, data=my_data_one)
summary(one.aov)
```
```{r}
two.aov<- aov(len ~ supp, data=my_data_two)
summary(two.aov)
```
At 2mg dose, supplement form is NOT significant
At 1mg dose, supplement form is significant (VC is longer than OJ)
At 0.5mg dose, supplement form is significant (VC is longer than OJ)


For VC form, dose is significant (higher dose, longer)
For OJ form, dose is only significant between 0.5mg and (1mg and 2mg) The difference between 1mg and 2mg is not significant
