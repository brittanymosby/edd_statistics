...
tpval<-2 * pt(ttstat, df=69, lower.tail=TRUE)
tpval
```
...
```{r}
tcollege <- (30500-30000)/(4500/sqrt(14))
...
```{r}
2*pt(tcollege, df=13, lower.tail = FALSE)
```

FAIL TO REJECT NULL HYPOTHESIS. There is not sufficient evidence that the private college tuition is different from $30,000.



A simple random sample of 40 recorded speeds is obtained from cars traveling on a section of highway. The sample has a mean 68.4 mph and a standard deviation of 5.7 mph. Test the claim that the average speed of all cars is not 65 mph, the posted speed limit.
...
```{r}
tspeed <- (68.4-65)/(5.7/sqrt(40))
tspeed
```
```{r}
qt(.025, 39)

2*pt(tspeed, 39, lower.tail = FALSE)
```

REJECT THE NULL HYPOTHESIS. 

The average speed (M = 68.4) of cars is significantly different from 65 mph, t(39)=3.77, p<0.05.
