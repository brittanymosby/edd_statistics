...
```{r}
bones <- read.table("bones.txt", header=T, sep="\t")
head(bones)
```

```{r}
bones.anova <- aov(Calcium ~ Density, data=bones)
summary(bones.anova)
```
A one-way analysis of variance revealed no significant difference in mean daily calcium intake in adults with varying bone density, *F*(2,15) = 1.40, p > 0.05.
